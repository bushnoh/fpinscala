...
sealed trait Option[+A] {
  ...
  def orElse[B >: A](ob: => Option[B]): Option[B] = this map (e => Some(e)) getOrElse (ob)
...
object Option {
  ...
  def variance(xs: Seq[Double]): Option[Double] = {
    //mean(xs.flatMap(x => math.pow(x-mean(xs),2)))
    mean(xs).flatMap(mXs => mean(xs.map(x => math.pow(x - mXs, 2))))
  }

  def map2[A, B, C](a: Option[A], b: Option[B])(f: (A, B) => C): Option[C] = a.flatMap((a1: A) => b.map((b1: B) => f(a1, b1)))

  def sequence[A](a: List[Option[A]]): Option[List[A]] = {
    val nil: Option[List[A]] = Some(Nil)
    a.foldRight(nil)((a, b) => map2(a, b)(_ :: _))
  }


  def traverse[A, B](a: List[A])(f: A => Option[B]): Option[List[B]] = {
    val nil: Option[List[B]] = Some(Nil)
    a.foldRight(nil)((a, z) => f(a) match {
      case Some(b) => map2(Some(b),z)(_::_)
      case None => None
    })
  }
