...
sealed trait Either[+E, +A] {
  ...
  def map22[EE >: E, B, C](b: Either[EE, B])(f: (A, B) => C): Either[EE, C] = for (aa <- this; bb <- b) yield f(aa, bb)
...
object Either {
  def traverse[E, A, B](es: List[A])(f: A => Either[E, B]): Either[E, List[B]] = {
    val nil: Either[E, List[B]] = Right(Nil)
    es.foldRight(nil)((a,z) => z.flatMap(zz => f(a).map(aa => aa::zz)))
  }
